...
class AGMA(nn.Module):
    ...
    def __init__(self, modalities: Sequence[str], cfg: Dict[str, Any]) -> None:
        ...
        self.contrast_same_class_positives = bool(m.get("contrast_same_class_positives", False))
        ...
        self.modality_prior_logit_weight = float(m.get("modality_prior_logit_weight", 0.0))
        self.reference_modality = str(m.get("reference_modality", "mmwave")).lower()
        self.reference_agreement_weight = float(m.get("reference_agreement_weight", 0.0))
        self.reference_residual_weight = float(m.get("reference_residual_weight", 0.0))
        self.context_prior_weight = float(m.get("context_prior_weight", 0.0))
        pam_cfg = m.get("positive_anchor_mining", {}) or {}
        self.positive_anchor_enabled = bool(pam_cfg.get("enabled", False))
        self.positive_reference_modality = str(pam_cfg.get("reference_modality", self.reference_modality)).lower()
        self.positive_threshold = float(pam_cfg.get("threshold", 0.15))
        self.positive_temperature = float(pam_cfg.get("temperature", 0.10))
        self.positive_min_weight = float(pam_cfg.get("min_weight", 0.05))
        self.positive_detach_reference = bool(pam_cfg.get("detach_reference", True))
        self.positive_gate_weight = float(pam_cfg.get("positive_gate_weight", 0.0))
        self.positive_alignment_weight = float(pam_cfg.get("positive_alignment_weight", 0.0))
        self.positive_aux_weight = float(pam_cfg.get("positive_aux_weight", 0.0))
        self.modality_prior = {
    ...
    def _sample_context(self, token_dict: Dict[str, torch.Tensor], active: Sequence[str]) -> torch.Tensor:
        ...
        stack = torch.stack(summaries, dim=1)
        if len(active) <= 1 or self.context_prior_weight <= 0:
            return stack.mean(dim=1)
        prior = stack.new_tensor([max(self.modality_prior.get(m, 1.0), 1e-8) for m in active])
        prior = prior / prior.sum().clamp_min(1e-8)
        uniform = prior.new_full(prior.shape, 1.0 / prior.numel())
        blend = max(0.0, min(1.0, self.context_prior_weight))
        weights = (1.0 - blend) * uniform + blend * prior
        return (stack * weights.view(1, -1, 1)).sum(dim=1)
    ...
    def _anchor_aggregate(
        ...
        strength = token_to_anchor.mean(dim=1)
    ...
    def _alignment_loss(
        ...
        weight = (strengths.detach() * modality_weights.detach()).clamp_min(1e-4)
        if positive_weights is not None and self.positive_alignment_weight > 0:
            pos = positive_weights.detach().clamp_min(1e-8)
            blend = max(0.0, min(1.0, self.positive_alignment_weight))
            weight = weight * ((1.0 - blend) + blend * pos)
    ...
    def _contrastive_alignment_loss(
        ...
        if self.contrast_same_class_positives and targets is not None and targets.numel() == bsz:
    ...
    def _positive_anchor_weights(
        self,
        stack: torch.Tensor,
        active: Sequence[str],
    ) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ref_idx = active.index(self.positive_reference_modality) if self.positive_reference_modality in active else -1
        weights = stack.new_ones(stack.shape[:3])
        agreement = stack.new_zeros(stack.shape[:3])
        if not self.positive_anchor_enabled or stack.size(2) <= 1 or ref_idx < 0:
            return weights, agreement, ref_idx

        z = F.normalize(stack, dim=-1)
        ref = z[:, :, ref_idx]
        if self.positive_detach_reference:
            ref = ref.detach()
        agreement = (z * ref.unsqueeze(2)).sum(dim=-1)

        temp = max(self.positive_temperature, 1e-6)
        raw = torch.sigmoid((agreement - self.positive_threshold) / temp)
        floor = max(0.0, min(self.positive_min_weight, 1.0))
        weights = floor + (1.0 - floor) * raw
        ref_mask = torch.zeros_like(weights)
        ref_mask[:, :, ref_idx] = 1.0
        weights = weights * (1.0 - ref_mask) + ref_mask
        return weights, agreement, ref_idx
    ...
    def _fuse_modalities(
        ...
        if self.modality_prior_logit_weight > 0:
            prior = gate_logits.new_tensor([max(self.modality_prior.get(m, 1.0), 1e-8) for m in active])
            prior = prior / prior.sum().clamp_min(1e-8)
            gate_logits = gate_logits + self.modality_prior_logit_weight * prior.log().view(1, 1, -1)
        ref_idx = active.index(self.reference_modality) if self.reference_modality in active else -1
        if self.reference_agreement_weight > 0 and ref_idx >= 0:
            ref = z[:, :, ref_idx].detach()
            ref_agreement = (z * ref.unsqueeze(2)).sum(dim=-1)
            gate_logits = gate_logits + self.reference_agreement_weight * ref_agreement
        if positive_weights is not None and self.positive_gate_weight > 0:
            gate_bias = positive_weights
            if ref_idx >= 0:
                ref_mask = torch.zeros_like(gate_bias)
                ref_mask[:, :, ref_idx] = 1.0
                gate_bias = gate_bias * (1.0 - ref_mask)
            gate_logits = gate_logits + self.positive_gate_weight * gate_bias
        ...
        fused = (weights.unsqueeze(-1) * stack).sum(dim=2)
        if self.reference_residual_weight > 0 and ref_idx >= 0:
            residual = max(0.0, min(1.0, self.reference_residual_weight))
            fused = (1.0 - residual) * fused + residual * stack[:, :, ref_idx]
    ...
    def forward(
        ...
        positive_weights, positive_agreement, _ = self._positive_anchor_weights(stack, active)
        aux_logits: Dict[str, torch.Tensor] = {}
        if self.anchor_aux_enabled and self.training:
        ...
        fused_anchors, modality_weights = self._fuse_modalities(stack, active, positive_weights)
        ...
        if align_w > 0 and len(active) > 1:
            align = self._alignment_loss(stack, strengths, modality_weights, positive_weights)
        ...
        for idx, modality in enumerate(active):
            ...
            loss_stats[f"positive_anchor_weight_{modality}"] = positive_weights.detach()[:, :, idx].mean()
            loss_stats[f"positive_anchor_agreement_{modality}"] = positive_agreement.detach()[:, :, idx].mean()
            loss_stats[f"positive_anchor_enhanced_{modality}"] = (
                positive_agreement.detach()[:, :, idx] > self.positive_threshold
            ).float().mean()
        loss_stats["modality_gate_min"] = modality_weights.detach().amin()
        loss_stats["modality_gate_max"] = modality_weights.detach().amax()
        loss_stats["positive_anchor_weight_min"] = positive_weights.detach().amin()
        loss_stats["positive_anchor_weight_max"] = positive_weights.detach().amax()
        ...
        if aux_logits:
            ...
            if self.positive_aux_weight > 0:
                out["aux_loss_sample_weights"] = {
                    modality: (
                        1.0
                        - max(0.0, min(1.0, self.positive_aux_weight))
                        + max(0.0, min(1.0, self.positive_aux_weight))
                        * positive_weights.detach()[:, :, idx].mean(dim=1)
                    )
                    for idx, modality in enumerate(active)
                    if modality in aux_logits
                }
        if return_debug:
            out["debug"] = {
                "active": active,
                ...
                "positive_anchor_weights": positive_weights.detach(),
                "positive_anchor_agreement": positive_agreement.detach(),
